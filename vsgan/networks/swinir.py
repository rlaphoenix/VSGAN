...
class SwinIR(nn.Module):
    def __init__(
        ...
        """
        A PyTorch implementation of SwinIR: Image Restoration Using Swin Transformer, based on Swin Transformer.
        ...
        # number of features
        num_feat = (
            self.state["conv_before_upsample.0.weight"].shape[1]
            if self.state.get("conv_before_upsample.weight")
            else 64
        )
        # number of input channels
        num_in_ch = self.state["conv_first.weight"].shape[1]
        # number of output channels
        if "conv_last.weight" in self.state:
        ...
        # window size
        window_size = int(math.sqrt(
            self.state["layers.0.residual_group.blocks.0.attn.relative_position_index"].shape[0]
        ))
        # input image size
        ...
        # image range, jpeg models are the only ones with window-size 7, and they also use this range
        ...
        if resi_connection == "1conv":
            ...
            raise ValueError(f"Unrecognized resi connection, '{resi_connection}'")
        ...
        if self.upsampler == "pixelshuffle":
            ...
            if self.upscale != 4:
                raise ValueError("The upsampler 'nearest+conv' only supports 4x upscaling, for now...")
            self.conv_before_upsample = nn.Sequential(
...
# Layers only used by SwinIR

class PatchMerging(nn.Module):
    def __init__(self, input_resolution: tuple[int, int], dim: int, norm_layer: Optional[nn.Module] = nn.LayerNorm):
        """
        Patch Merging Layer.
    ...
    def forward(self, x):
        ...
        if h % 2 != 0 or w % 2 != 0:
            raise ValueError(f"Size of x ({h}*{w}) is not even.")

        b, seq_len, c = x.shape
        if seq_len != h * w:
            raise ValueError(f"Input feature has wrong size, expected {h * w} got {seq_len}")
...
class UpsampleOneStep(nn.Sequential):
    def __init__(self, scale: int, num_feat: int, num_out_ch: int, input_resolution: tuple[int, int]):
        """
        UpsampleOneStep module (only has 1conv + 1pixelshuffle)
...
__ALL__ = (SwinIR, PatchMerging, BasicLayer, RSTB, SwinTransformerBlock, WindowAttention, PatchEmbed, PatchUnEmbed,
           Upsample, UpsampleOneStep, DropPath, Mlp, drop_path)
